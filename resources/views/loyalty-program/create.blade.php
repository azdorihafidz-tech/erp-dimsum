...
@section('title', 'Tambah Program Loyalty')

@section('content')
<div class="d-flex justify-content-between align-items-center mb-3">
    <h4 class="mb-0 fw-bold"><i class="bi bi-award me-2 text-primary"></i>Tambah Program Loyalty</h4>
...
<div class="card">
    <div class="card-body">
        <form method="POST" action="{{ route('loyalty-program.store') }}">
            @csrf
            <div class="row g-3">
                <div class="col-12">
                ...
                <div class="col-12">
                    ...
                    <textarea name="hadiah" class="form-control" rows="2" required placeholder="mis. Beras 5kg, Minyak Goreng 2L / Voucher Rp 15.000">{{ old('hadiah') }}</textarea>
                </div>

                <div class="col-12 col-md-4 field-event-based">
                    <label class="form-label">Nominal Voucher Default (Rp)</label>
                    <input type="number" step="1" min="0" name="nominal_voucher" class="form-control" value="{{ old('nominal_voucher') }}" placeholder="mis. 15000">
                ...
                <div class="col-12 col-md-4 field-auto-track">
                    ...
                    <select name="tipe_item" class="form-select">
                        <option value="penjualan" {{ old('tipe_item','penjualan')==='penjualan'?'selected':'' }}>Penjualan (D'mentai)</option>
                        <option value="jasa_giling" {{ old('tipe_item')==='jasa_giling'?'selected':'' }}>Jasa Giling (legacy)</option>
                    </select>
                </div>
                <div class="col-12 col-md-4 field-auto-track">
                    <label class="form-label">Basis Perhitungan</label>
                    <select name="sumber_data" class="form-select">
                        <option value="orders.total_bayar" {{ old('sumber_data','orders.total_bayar')==='orders.total_bayar'?'selected':'' }}>Total Belanja (Rp)</option>
                        <option value="orders.count" {{ old('sumber_data')==='orders.count'?'selected':'' }}>Jumlah Transaksi</option>
                        <option value="orders.berat_daging_kg" {{ old('sumber_data')==='orders.berat_daging_kg'?'selected':'' }}>Berat Gilingan (kg, legacy)</option>
                    </select>
                    <small class="text-muted">Menentukan satuan Target di atas — pilih sesuai cara kamu mau ukur loyalitas pelanggan.</small>
                </div>
            ...
            <a href="{{ route('loyalty-program.index') }}" class="btn btn-outline-secondary ms-2">Batal</a>
        </form>
    </div>
</div>
